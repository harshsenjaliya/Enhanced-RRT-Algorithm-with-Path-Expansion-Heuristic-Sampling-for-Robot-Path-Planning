...
def extend(from_node, to_point, step_size, obstacle_space):
    ...
    while np.linalg.norm(current_node.point - to_point) >= step_size:
    ...
    return path, Node(to_point, current_node)  # Ensure the final step towards the goal is also taken
...
def rrt_star_connect(start, goal, obstacle_space, num_iterations=5000, step_size=30, threshold=20, goal_bias=0.4):
    ...
    for i in range(num_iterations):
        random_point = goal if np.random.rand() < goal_bias else np.random.rand(2) * obstacle_space.shape[::-1]
